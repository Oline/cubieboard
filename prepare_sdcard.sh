...
" | fdisk "$SDCARD_DEVICE"
		...
		echo "Not formating \"$SDCARD_DEVICE\" device. I assume you already did it correctly."
            ...
            # format the first 1M of the sdcard to write fresh bootloader there
	    # THIS ERASE THE PARTITION TABLE...
            # echo "Formating the first 1M of the sdcard to write fresh bootloader there."
...
# copy boot binaries to the sdcard
	...
	then
	    ...
	    echo "$SDCARD_DEVICE does not seem to be a block device..."
	fi
    else
	echo "The variable SDCARD_DEVICE does not seem to exist..."
    fi
}
...
case "$1" in
	...
	;;
    *)
	echo "Usage: prepare_sdcard.sh {all|format|copyboot2sdcard|copyrootfs2sdcard}"
	exit 1
esac
